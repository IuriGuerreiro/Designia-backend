...
if __name__ == '__main__':
    ...
    if not cert_file.exists() or not key_file.exists():
        ...
        print("cd ../certs && openssl req -x509 -newkey rsa:4096 -keyout key.pem -out cert.pem -days 365 -nodes")
        sys.exit(1)

    # Run Django with HTTPS
    print(f"Starting Django HTTPS server...")
    print(f"Using cert: {cert_file}")
    print(f"Using key: {key_file}")
    print(f"Server will be available at: https://0.0.0.0:8001")
    print(f"Access via: https://192.168.3.2:8001 or https://localhost:8001")
    print("\nNote: You may see browser security warnings with self-signed certificates.")
    print("This is normal for development. Click 'Advanced' and 'Proceed' to continue.\n")
