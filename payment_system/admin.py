...
class PaymentTransactionAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'purchase_date', 'days_remaining_display'
    ]
    
    list_filter = [
        'status', 'hold_reason', 'currency', 'purchase_date', 'payment_received_date'
    ]
    
    search_fields = [
        'stripe_payment_intent_id', 'stripe_checkout_session_id',
        'seller__username', 'seller__email', 'buyer__username', 'buyer__email',
        'item_names', 'notes', 'hold_notes'
    ]
    
    readonly_fields = [
        'id', 'stripe_payment_intent_id', 'stripe_checkout_session_id',
        'net_amount', 'purchase_date', 'payment_received_date', 'planned_release_date',
    ...
    fieldsets = (
        ...
        ('Hold System', {
            'fields': ('hold_reason', 'days_to_hold', 'hold_start_date', 'planned_release_date', 'actual_release_date', 'released_by', 'hold_notes')
        }),
        ('Dates', {
            'fields': ('purchase_date', 'payment_received_date')
        }),
        ('Additional Info', {
    ...
    def hold_status(self, obj):
        if obj.status == 'held':
            if obj.can_be_released:
                return format_html(
                    '<span style="color: red;">Ready for release</span>'
                )
            else:
                days_left = obj.days_remaining
                return format_html(
                    '<span style="color: orange;">Held ({} days remaining)</span>',
                    days_left
                )
