...
class CurrencyHandler:
    ...
    def get_stripe_balance() -> Dict:
        ...
        """
        try:
            logger.debug(f"[STRIPE_DEBUG] Retrieving Stripe account balance...")
            balance = stripe.Balance.retrieve()
            
            logger.debug(f"[STRIPE_DEBUG] Stripe balance retrieved successfully")
    ...
    def get_available_currencies_with_balance() -> List[Dict]:
        ...
        """
        try:
            logger.debug(f"[BALANCE_DEBUG] Getting available currencies with balance...")
            ...
            for i, currency_balance in enumerate(balance.available):
                ...
                logger.debug(f"[BALANCE_DEBUG] Processing currency[{i}]: {currency} = {amount_cents} cents ({amount_decimal:.2f})")
            ...
            logger.debug(f"[BALANCE_DEBUG] Found {currencies_with_balance} currencies with positive balance out of {total_available} total")
            
            # Sort by amount (highest first), then by preference
            logger.debug(f"[BALANCE_DEBUG] Sorting currencies by amount and preference...")
            
            def sort_key(x):
                amount_key = x['amount_cents']
                preference_key = -CurrencyHandler.PREFERRED_CURRENCIES.index(x['currency_code']) if x['currency_code'] in CurrencyHandler.PREFERRED_CURRENCIES else 999
                logger.debug(f"[BALANCE_DEBUG] Sort key for {x['currency']}: amount={amount_key}, preference={preference_key}")
                return (amount_key, preference_key)
            
            available_currencies.sort(key=sort_key, reverse=True)
            
            logger.debug(f"[BALANCE_DEBUG] Final sorted order:")
            ...
            logger.error(f"[BALANCE_DEBUG] Exception in get_available_currencies_with_balance: {type(e).__name__}: {e}")
        ...
        try:
            ...
            if base_rates_count > 0:
                ...
                recent_rates = ExchangeRate.objects.filter(
                    base_currency=base_currency_upper
                ).order_by('-created_at')[:5]
                
                for rate in recent_rates:
                    logger.debug(f"[DB_DEBUG] Rate found: {rate.base_currency}->{rate.target_currency}={rate.rate} (created: {rate.created_at})")
            
            # Get rates from database
            rates = ExchangeRate.objects.get_latest_rates(base_currency_upper)
            
            logger.debug(f"[DB_DEBUG] get_latest_rates returned: {type(rates)} with {len(rates) if rates else 0} items")
            if rates:
            ...
            if rates:
                ...
                logger.error(f"[DB_DEBUG] Total rates in DB: {total_rates}, Rates for {base_currency_upper}: {base_rates_count}")
            ...
            raise ValueError(f"Failed to retrieve exchange rates for {base_currency}: {str(e)}")
        ...
        try:
            ...
            if latest_rate:
                ...
                total_rates = ExchangeRate.objects.filter(
                    created_at=latest_rate.created_at
                ).count()
    ...
    def convert_currency(amount: Decimal, from_currency: str, to_currency: str) -> Tuple[Decimal, float]:
        ...
        try:
            ...
            if target_rate is None:
                logger.error(f"[CONVERT_DEBUG] No exchange rate found for {from_currency} -> {to_currency} in database")
                raise ValueError(f"Exchange rate not available for {from_currency} to {to_currency}. Please update exchange rate data.")
            
            converted_amount = amount * Decimal(str(target_rate))
            
            logger.info(f"Converted {amount} {from_currency} to {converted_amount:.2f} {to_currency} (rate: {target_rate})")
            logger.debug(f"[CONVERT_DEBUG] Conversion successful using targeted lookup")
            ...
            for currency_balance in balance.available:
                if currency_balance['currency'].lower() == currency.lower():
                    ...
                    return {
                        ...
                        'shortage_decimal': max(Decimal('0'), (Decimal(required_amount_cents) - Decimal(available_cents)) / 100)
                    }
    ...
    @staticmethod
    def find_optimal_currency_for_transfer(
        ...
        if not available_currencies:
            ...
            return {
                'success': False,
                'error': 'No currencies with available balance',
                'balance_info': preferred_check
            }
        ...
        for currency_info in available_currencies:
            ...
            if available_cents >= converted_amount_cents:
                logger.info(f"Found suitable currency: {alt_currency} (converted amount: {converted_amount_cents} cents)")
...
def switch_currency(
    ...
    try:
        ...
        currency_result = CurrencyHandler.find_optimal_currency_for_transfer(
            preferred_currency, required_amount_cents
        )
        ...
        try:
            ...
            logger.info(f"Destination account country: {dest_country}, selected currency: {currency_result['use_currency']}")
        ...
        if currency_result['conversion_needed']:
            logger.info(f"Currency conversion needed: {currency_result['original_currency']} to {currency_result['use_currency_display']} (rate: {currency_result['exchange_rate']})")
