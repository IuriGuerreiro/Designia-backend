...
from payment_system.models import (
    PaymentTransaction, Payout, PayoutItem, PaymentTracker
)
from marketplace.models import Order, Product, OrderItem, Category, Cart, CartItem
...
class PaymentHoldsEndpointTests(BaseAuthTestCase):
    ...
    def setUp(self):
        ...
        self.transaction = self.factory.create_payment_transaction(
            self.seller, self.buyer, self.order, status='held'
        )
...
class AdminPayoutEndpointTests(BaseAuthTestCase):
    ...
    def setUp(self):
        ...
        self.transaction1 = self.factory.create_payment_transaction(
            self.seller, self.buyer, order1, 'completed'
        )
        ...
        self.transaction2 = self.factory.create_payment_transaction(
            self.seller2, self.buyer, order2, 'held'
        )
...
class EdgeCaseAndErrorHandlingTests(BaseAuthTestCase):
    ...
    def test_malformed_json(self):
        ...
        response = self.client.post(
            url,
            data='{"amount": "not-a-number", "currency": }',  # Invalid JSON
