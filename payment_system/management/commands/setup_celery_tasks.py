...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help='Minute for daily exchange rate update (0-59, default: 0)'
        )
        parser.add_argument(
            '--timeout-interval',
            type=int,
            default=1,
            help='Hours between payment timeout checks (default: 1)'
        )
        parser.add_argument(
            '--force',
            action='store_true',
            help='Force update of existing tasks'
        )
        parser.add_argument(
            '--status-only',
            action='store_true',
            help='Show current task status without making changes'
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(f'Failed to set up Celery tasks: {e}')
    ...
    def _setup_tasks_with_options(self, options):
        ...
        results['exchange_rates'] = CelerySchedulerService.schedule_daily_exchange_rates(
            hour=options['exchange_hour'],
            minute=options['exchange_minute']
        )
