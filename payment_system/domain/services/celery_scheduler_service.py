...
class CelerySchedulerService:
    ...
    def setup_default_tasks(cls) -> Dict[str, bool]:
        ...
        try:
            ...
            return {"error": str(e), "exchange_rates": False, "payment_timeouts": False}
    ...
    def get_task_status(cls) -> Dict:
        ...
        """
        try:
            tasks = PeriodicTask.objects.filter(task__startswith="payment_system.Tasks.")
            ...
            for task in tasks:
                ...
                if task.crontab:
                    ...
                    task_info["schedule"] = (
                        f"{task.crontab.minute} {task.crontab.hour} {task.crontab.day_of_week} {task.crontab.day_of_month} {task.crontab.month_of_year}"
                    )
                elif task.interval:
            ...
            return {"error": str(e), "total_tasks": 0, "enabled_tasks": 0, "disabled_tasks": 0, "tasks": []}
    ...
    def trigger_manual_update(cls, task_name: str = "exchange_rates") -> Dict:
        ...
        """
        try:
            from ..Tasks import check_payment_timeouts_task, update_exchange_rates_task

            task_map = {"exchange_rates": update_exchange_rates_task, "payment_timeouts": check_payment_timeouts_task}
            ...
            return {"success": False, "error": error_msg, "task_name": task_name}
    ...
    def cleanup_old_tasks(cls, days: int = 30) -> Dict:
        ...
        try:
            ...
            deleted_count = PeriodicTaskChanged.objects.filter(last_update__lt=cutoff_date).delete()[0]
