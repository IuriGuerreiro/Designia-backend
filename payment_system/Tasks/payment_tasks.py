...
def cancel_expired_order(self, order_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            for order_item in order.items.all():
                try:
                    ...
                    if product and product.is_active:
                        ...
                        logger.info(f"Restored {order_item.quantity} units of {order_item.product_name} to stock. Stock: {original_stock} -> {product.stock_quantity}")
                    else:
                        logger.warning(f"Product {order_item.product_name} not found or inactive - cannot restore stock")
            ...
            transactions = PaymentTransaction.objects.filter(order=order).exclude(
                status__in=['completed', 'released', 'refunded', 'failed', 'cancelled']
            ).select_for_update()
            ...
            for payment_txn in transactions:
                try:
                    ...
                    if hasattr(payment_txn, 'cancel_payment'):
                        ...
                        payment_txn.payment_failure_reason = 'Payment timeout - Order cancelled after 3-day grace period'
                        payment_txn.save(update_fields=[
                            'status', 'payment_failure_code', 'payment_failure_reason', 'updated_at'
                        ])
            ...
            logger.info(f"Successfully cancelled expired order {order_id}, restored {len(restored_items)} products, and cancelled {cancelled_transactions} transactions")
...
def check_payment_timeouts_task(self):
    ...
    try:
        ...
        logger.info(f"Payment timeout check completed. Cancelled: {len(results['cancelled_orders'])}, Stock restored: {results['total_stock_restored']}, Transactions cancelled: {results['total_transactions_cancelled']}, Errors: {len(results['errors'])}")
