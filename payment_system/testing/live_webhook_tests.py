...
class LiveWebhookTester:
    ...
    def setup_webhook_forwarding(self):
        ...
        try:
            ...
            self.webhook_process = subprocess.Popen(
                ["stripe", "listen", "--forward-to", self.webhook_url, "--print-secret"],
    ...
    def trigger_webhook(self, event_type, description=""):
        ...
        try:
            result = subprocess.run(["stripe", "trigger", event_type], capture_output=True, text=True, timeout=30)
    ...
    def check_webhook_received(self, event_type, timeout=10):
        ...
        while time.time() - start_time < timeout:
            ...
            recent_events = WebhookEvent.objects.filter(
                event_type=event_type, created_at__gte=datetime.now().replace(second=0, microsecond=0)
            ).order_by("-created_at")
    ...
    def run_payment_webhook_test(self):
        ...
        # Trigger the webhook
        if not self.trigger_webhook("payment_intent.succeeded", "Tests payment processing completion"):
            return False
        ...
        if received:
            self.test_results.append(
                {"test": "payment_intent.succeeded", "status": "PASSED", "event_id": event.stripe_event_id}
            )
            return True
        else:
            self.test_results.append(
                {"test": "payment_intent.succeeded", "status": "FAILED", "error": "Webhook not received"}
            )
            return False
    ...
    def run_payment_failed_webhook_test(self):
        ...
        if not self.trigger_webhook("payment_intent.payment_failed", "Tests payment failure handling"):
            return False
        ...
        if received:
            self.test_results.append(
                {"test": "payment_intent.payment_failed", "status": "PASSED", "event_id": event.stripe_event_id}
            )
            return True
        else:
            self.test_results.append(
                {"test": "payment_intent.payment_failed", "status": "FAILED", "error": "Webhook not received"}
            )
            return False
    ...
    def run_account_webhook_test(self):
        ...
        if not self.trigger_webhook("account.updated", "Tests seller account status updates"):
            return False
        ...
        if received:
            self.test_results.append(
                {"test": "account.updated", "status": "PASSED", "event_id": event.stripe_event_id}
            )
            return True
        else:
            self.test_results.append({"test": "account.updated", "status": "FAILED", "error": "Webhook not received"})
            return False
    ...
    def run_transfer_webhook_test(self):
        ...
        if not self.trigger_webhook("transfer.created", "Tests seller payout processing"):
            return False
        ...
        if received:
            self.test_results.append(
                {"test": "transfer.created", "status": "PASSED", "event_id": event.stripe_event_id}
            )
            return True
        else:
            self.test_results.append({"test": "transfer.created", "status": "FAILED", "error": "Webhook not received"})
            return False
    ...
    def print_test_summary(self):
        ...
        print(
            f"   Recent events (last hour): {WebhookEvent.objects.filter(created_at__gte=datetime.now().replace(minute=0, second=0, microsecond=0)).count()}"
        )
        print()
