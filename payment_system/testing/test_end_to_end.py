...
class CompletePaymentFlowTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.buyer = User.objects.create_user(username="buyer", email="buyer@marketplace.com", password="testpass123")

        self.seller1 = User.objects.create_user(
            username="seller1", email="seller1@marketplace.com", password="testpass123"
        )

        self.seller2 = User.objects.create_user(
            username="seller2", email="seller2@marketplace.com", password="testpass123"
        )

        self.admin = User.objects.create_user(
            username="admin", email="admin@marketplace.com", is_staff=True, is_superuser=True
        )
        ...
        self.product1 = Product.objects.create(
            name="Product 1", description="First product", price=Decimal("50.00"), seller=self.seller1
        )

        self.product2 = Product.objects.create(
            name="Product 2", description="Second product", price=Decimal("75.00"), seller=self.seller2
        )
    ...
    def test_complete_successful_payment_flow(self, mock_payment_intent):
        ...
        mock_payment_intent.return_value = MagicMock(
            id="pi_complete_test_123", status="succeeded", client_secret="pi_complete_test_123_secret"
        )
        ...
        response = self.client.post(
            ...
            {"order_id": str(self.order.id), "payment_method_id": "pm_test_card"},
            format="json",
        )
    ...
    def test_complete_refund_flow(self, mock_refund_create, mock_payment_intent):
        ...
        mock_payment_intent.return_value = MagicMock(id="pi_refund_test_123", status="succeeded")
        ...
        mock_refund_create.return_value = MagicMock(id="re_test_123", status="succeeded")
    ...
    def test_seller_payout_processing(self, mock_transfer_create):
        ...
        mock_transfer_create.return_value = MagicMock(id="tr_test_123", status="pending")
...
class PaymentSecurityE2ETestCase(APITestCase):
    ...
    def setUp(self):
        self.buyer = User.objects.create_user(username="buyer", email="buyer@test.com")
        self.seller = User.objects.create_user(username="seller", email="seller@test.com")
    ...
    def test_payment_amount_tampering_prevention(self):
        ...
        with patch("stripe.PaymentIntent.create") as mock_payment_intent:
            mock_payment_intent.return_value = MagicMock(id="pi_tamper_test", status="succeeded")
...
class PerformanceE2ETestCase(TransactionTestCase):
    ...
    def setUp(self):
        ...
        for i in range(10):
            buyer = User.objects.create_user(username=f"buyer{i}", email=f"buyer{i}@test.com")
    ...
    def test_webhook_processing_performance(self):
        ...
        WebhookEvent.objects.filter(stripe_event_id__startswith="evt_perf_test").update(
        ...
        processed_count = WebhookEvent.objects.filter(
            stripe_event_id__startswith="evt_perf_test", status="processed"
        ).count()
...
class DataConsistencyE2ETestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.product = Product.objects.create(
            name="Consistency Test Product", price=Decimal("100.00"), seller=self.seller
        )
    ...
    def test_payment_payout_amount_consistency(self):
        ...
        self.assertEqual(total_payout_amount + total_fees, payment.amount)
    ...
    def test_refund_amount_validation(self):
        ...
        is_valid, error = PaymentValidator.validate_refund_eligibility(payment, refund2.amount)
