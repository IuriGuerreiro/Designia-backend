...
class StripePaymentTracker(models.Model):
    ...
    user = models.ForeignKey(
        User, 
        on_delete=models.PROTECT,
        related_name='stripe_payment_trackers',
        db_index=True
    )
    ...
    def is_suspicious(self):
        ...
        return (
            self.risk_level in ['high', 'blocked'] or
            (self.fraud_score and self.fraud_score > 75) or
            self.status in ['disputed', 'failed'] or
            self.failure_reason in ['card_declined', 'insufficient_funds', 'fraudulent']
        )
...
class WebhookEvent(models.Model):
    ...
    payment_tracker = models.ForeignKey(
        StripePaymentTracker,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='webhook_events_log'
    )
