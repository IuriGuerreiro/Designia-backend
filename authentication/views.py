...
class PublicProfileDetailView(generics.RetrieveAPIView):
    ...
    def get_object(self):
        ...
        # Allow if target user is a seller (admins count) or has verified seller profile
        is_target_seller = False
        try:
            is_target_seller = is_seller(user) or (
                hasattr(user, "profile") and getattr(user.profile, "is_verified_seller", False)
            )
        except Exception:
            is_target_seller = False
...
class ProfileUpdateView(generics.RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        # Debug logging (sanitized): do not log raw payload or field names
        try:
            fields_count = len(getattr(request, "data", {}) or {})
        except Exception:
            fields_count = 0
        logger.info(
            "Profile update requested for user_id=%s (fields=%s)",
            request.user.id,
            fields_count,
        )
        ...
        # Use centralized RBAC logic for admin privileges (DB-verified)
        has_admin_privileges = is_admin(instance)
        ...
        try:
            ...
            # Avoid logging full serializer errors to protect sensitive fields
            logger.error("Serializer validation failed for profile update", exc_info=True)
            raise
        ...
        # Return without logging response payload
...
def register(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            if isinstance(email_result, tuple):
                ...
                if email_sent:
                    ...
                    if "wait" in message.lower() and "seconds" in message.lower():
                        # Rate limit error
                        return Response(
                            {"error": message, "email": user.email, "email_sent": False},
            ...
            logger.error(
                f"Registration validation failed for request from {request.META.get('REMOTE_ADDR')}: {serializer.errors}"
            )
            ...
            return Response({"error": "Registration failed", "errors": errors}, status=status.HTTP_400_BAD_REQUEST)
    except Exception:
        return Response(
...
def login_verify_2fa(request):
    ...
    try:
        ...
        if not user_id or not code:
            return Response(
                {"error": "User ID and verification code are required"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    try:
        ...
        if isinstance(email_result, tuple):
            ...
            if email_result:
                ...
                return Response(
                    {"error": "Failed to send verification email"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
...
def check_email_rate_limit(request):
    ...
    try:
        ...
        try:
            ...
            return Response({"can_send": True, "time_remaining": 0}, status=status.HTTP_200_OK)

        can_send, time_remaining = get_email_rate_limit_status(user, email, request_type)

        return Response({"can_send": can_send, "time_remaining": time_remaining}, status=status.HTTP_200_OK)
...
def google_login(request):
    ...
    if serializer.is_valid():
        ...
        # Avoid logging full serializer errors (may include sensitive values)
        try:
            err_fields = list(serializer.errors.keys())
        except Exception:
            err_fields = []
        logger.error(f"Google login validation error (fields={err_fields})")
...
def verify_2fa(request):
    ...
    try:
        ...
        if purpose == "enable_2fa":
            ...
            user.save()
            return Response(
                {"message": "2FA has been successfully enabled for your account.", "two_factor_enabled": True},
                status=status.HTTP_200_OK,
            )
            ...
            user.save()
            return Response(
                {"message": "2FA has been successfully disabled for your account.", "two_factor_enabled": False},
                status=status.HTTP_200_OK,
            )
...
def get_2fa_status(request):
    ...
    try:
        user = request.user
        return Response(
            {"two_factor_enabled": user.two_factor_enabled, "email": user.email}, status=status.HTTP_200_OK
        )
    except Exception:
        return Response(
            {"error": "Service may be unavailable. Please try again later."},
...
def set_password_with_2fa(request):
    ...
    try:
        ...
        return Response(
            {"message": "Password has been successfully set for your account.", "is_oauth_only_user": False},
            status=status.HTTP_200_OK,
        )
...
def request_password_reset(request):
    ...
    try:
        ...
        if success:
            ...
            return Response({"error": result, "email_sent": False}, status=status.HTTP_429_TOO_MANY_REQUESTS)
...
def resend_2fa_code(request):
    ...
    try:
        ...
        if success:
            return Response(
                {"message": "A new verification code has been sent to your email.", "email": user.email},
                status=status.HTTP_200_OK,
            )
        else:
            return Response({"error": result}, status=status.HTTP_429_TOO_MANY_REQUESTS)
...
def upload_profile_picture(request):
    ...
    try:
        ...
        if not getattr(settings, "USE_S3", False):
            return Response({"error": "S3 storage is not enabled"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response(
                {"error": f"Failed to upload profile picture: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    try:
        ...
        try:
            ...
            if success:
                ...
                return Response(
                    {"error": "Failed to delete profile picture from S3"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )

        except S3StorageError as e:
            return Response(
                {"error": f"Failed to delete profile picture: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
