...
from .seller_serializers import SellerApplicationAdminSerializer, SellerApplicationSerializer, UserRoleSerializer
...
def seller_application_status(request):
    ...
    return Response({"has_application": False, "is_seller": is_seller(request.user), "status": None})
...
def apply_to_become_seller(request):
    ...
    try:
        ...
        existing = SellerApplication.objects.filter(user=request.user).order_by("-submitted_at").first()
        ...
        if existing and existing.status in ["pending", "under_review", "revision_requested"]:
            ...
            return Response(
                {"error": "You already have a seller application in progress."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if is_seller(request.user):
            ...
            return Response({"error": "You are already a verified seller."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        logger.error("=== SELLER APPLICATION ERROR ===")
        ...
        return Response(
            {"error": f"Failed to submit application: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def admin_approve_seller(request, application_id):
    ...
    try:
        ...
        return Response({"success": True, "message": f"Seller application approved for {application.user.email}"})
...
def admin_reject_seller(request, application_id):
    ...
    try:
        ...
        return Response({"success": True, "message": f"Seller application rejected for {application.user.email}"})
    except SellerApplication.DoesNotExist:
        return Response({"error": "Application not found"}, status=status.HTTP_404_NOT_FOUND)
