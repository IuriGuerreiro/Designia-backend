...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_typing_stop(self):
        ...
        logger.info(f"⏹️ User {self.user.username} (ID: {self.user.id}) stopped typing in chat {self.chat_id}")
        await self.channel_layer.group_send(
            self.room_group_name,
            {
                'type': 'typing_stop',
    ...
    async def typing_start(self, event):
        ...
        if event['user_id'] != self.user.id:
            logger.info(f"📤 Sending typing_start to user {self.user.username}: {event['username']} is typing")
            await self.send(text_data=json.dumps({
                'type': 'typing_start',
                'user_id': event['user_id'],
                'username': event['username'],
