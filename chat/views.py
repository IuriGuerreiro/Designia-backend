...
class ChatListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return Chat.objects.filter(Q(user1=user) | Q(user2=user)).select_related(
            "user1", "user2", "last_message__sender"
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if created:
                ...
                logger.info(
                    f"📬 New chat created between {request.user.username} and {other_user.username} (chat_id: {chat.id})"
                )
                ...
                logger = logging.getLogger(__name__)
                logger.info(
                    f"🔄 Existing chat returned between {request.user.username} and {other_user.username} (chat_id: {chat.id})"
                )
...
class MessageListView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if create_serializer.is_valid():
            # Create message
            message = Message.objects.create(chat=chat, sender=request.user, **create_serializer.validated_data)
            ...
            logger = logging.getLogger(__name__)
            logger.info(
                f"📤 New message in chat {chat.id}: {message.text_content[:50]}... -> notifying user {other_user.id}"
            )
...
def upload_chat_image(request):
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"error": f"Failed to upload image: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
def mark_messages_read(request, chat_id):
    ...
    try:
        ...
        if updated_count > 0:
            ...
            logger = logging.getLogger(__name__)
            logger.info(
                f"📖 Messages marked as read in chat {chat.id} by user {request.user.id} -> notifying user {other_user.id}"
            )
