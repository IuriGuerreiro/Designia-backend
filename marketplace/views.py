...
from activity.models import UserClick  # noqa: E402

# Import S3 storage utilities
from utils.s3_storage import S3StorageError, get_s3_storage  # noqa: E402

# Import transaction utilities
from .filters import ProductFilter  # noqa: E402
...
from .permissions import IsOwnerOrReadOnly, IsSellerOrReadOnly, IsSellerUser  # noqa: E402
...
# Set up logger
# (already defined above to be available during import guard)
...
class CategoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def products(self, request, slug=None):
        ...
        try:
            ...
            for product in products_list:
                try:
                    UserClick.track_activity(
                        product=product, action="category_view", user=user, session_key=session_key, request=request
                    )
                    tracked_count += 1
                except Exception as e:
            ...
            logger.info(
                f"Tracked category views for {tracked_count}/{len(products_list)} products in category {category.name}"
            )
        except Exception as e:
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if getattr(settings, "USE_S3", False):
            ...
            if all_images:
                try:
                    ...
                    for i, (image_type, image_file) in enumerate(all_images):
                        try:
                            ...
                            logger.info(
                                f"Image validated: {validated_image['random_name']} (was {validated_image['original_name']})"
                            )
                            ...
                            try:
                                ...
                                if all([encoding, quality, original_size, encoded_size, compression_ratio]):
                                    ...
                                    logger.info(
                                        f"No encoding metadata provided for image {i}, proceeding with basic validation"
                                    )
                            ...
                            logger.info(
                                f"Uploaded {image_type} image for product {product.id}: {result['key']} (DB record: {product_image.id})"
                            )
                ...
                logger.warning("This might indicate an issue with file detection or FormData processing")
    ...
    def _list_legacy(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            logger.info("=== PRODUCT LIST - AWS PRESIGNED URLS DEBUG ===")
            ...
            for i, product_data in enumerate(
                ...
                if primary_image:
                    ...
                    logger.info(
                        f"📋 LIST - AWS PRESIGNED URL for {product_data.get('name')}: {primary_image.get('presigned_url')}"
                    )
                else:
                    logger.info("  - No primary image found")
        ...
        logger.info("=== PRODUCT LIST (NON-PAGINATED) - AWS PRESIGNED URLS DEBUG ===")
    ...
    def _retrieve_legacy(self, request, *args, **kwargs):
        ...
        logger.info("=== PRODUCT RETRIEVE - AWS PRESIGNED URLS DEBUG ===")
    ...
    def click(self, request, slug=None):
        ...
        try:
            ...
            tracking_thread = threading.Thread(
                target=lambda: AsyncTracker.queue_product_click(product, request), daemon=True
            )
            tracking_thread.start()
        except Exception as e:
            logger.error(f"Error starting background tracking: {str(e)}")
    ...
    def images(self, request, slug=None):
        ...
        if not getattr(settings, "USE_S3", False):
            return Response({"detail": "S3 storage is not enabled", "images": []})
        ...
        try:
            ...
            return Response(
                {"detail": f"Failed to retrieve images: {str(e)}", "images": []},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def upload_image(self, request, slug=None):
        ...
        if not getattr(settings, "USE_S3", False):
            return Response({"detail": "S3 storage is not enabled"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if product.seller != request.user:
            return Response(
                {"detail": "You can only upload images to your own products"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        try:
            ...
            return Response(
                {"detail": f"Failed to upload image: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def delete_image(self, request, slug=None):
        ...
        if product.seller != request.user:
            return Response(
                {"detail": "You can only delete images from your own products"}, status=status.HTTP_403_FORBIDDEN
            )

        # Get image key from request
        image_key = request.data.get("image_key")
        if not image_key:
            return Response({"detail": "image_key is required"}, status=status.HTTP_400_BAD_REQUEST)

        try:
            s3_storage = get_s3_storage()
            success = s3_storage.delete_specific_product_image(product_id=str(product.id), image_key=image_key)

            if success:
                logger.info(f"User {request.user.username} deleted image {image_key} for product {product.id}")
                return Response({"success": True, "message": "Image deleted successfully", "deleted_key": image_key})
            else:
                return Response({"detail": "Failed to delete image"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        except S3StorageError as e:
            logger.error(f"Failed to delete image {image_key} for product {product.id}: {str(e)}")
            return Response(
                {"detail": f"Failed to delete image: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def delete_all_images(self, request, slug=None):
        ...
        try:
            ...
            return Response(
                {"detail": f"Failed to delete images: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def main_image(self, request, slug=None):
        ...
        if primary_image:
            ...
            logger.info("=== MAIN IMAGE ENDPOINT - AWS PRESIGNED URL ===")
            ...
            logger.info("=== MAIN IMAGE ENDPOINT - NO IMAGE FOUND ===")
            logger.info(f"Product: {product.name} (ID: {product.id}, Slug: {product.slug})")
...
class ProductReviewViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if product_slug:
            return ProductReview.objects.filter(product__slug=product_slug, is_active=True).select_related(
                "reviewer", "product"
            )
    ...
    def perform_create(self, serializer):
        product_slug = (
            self.request.data.get("product_slug")
            or self.request.query_params.get("product_slug")
            or self.kwargs.get("product_slug")
        )
        if not product_slug:
...
class CartViewSet(viewsets.ModelViewSet):
    ...
    def add_item(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                tracking_thread = threading.Thread(
                    target=lambda: AsyncTracker.queue_cart_action(
                        product, request.user, "cart_add", quantity, request
                    ),
                    daemon=True,
                )
                tracking_thread.start()
        ...
        error_response = {"error": "VALIDATION_ERROR", "detail": "Invalid data provided", "errors": serializer.errors}
    ...
    def update_item(self, request):
        ...
        return Response(
            {"success": True, "item": CartItemSerializer(cart_item).data, "message": "Cart item updated successfully"}
        )
    ...
    def clear(self, request):
        ...
        send_cart_update_notification(user_id=request.user.id, action="clear_cart", message="Cart cleared")
    ...
    def update_item_status(self, request):
        ...
        if not item_id:
            return Response(
                {"error": "MISSING_PARAMETERS", "detail": "item_id is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {"error": "ITEM_NOT_FOUND", "detail": "Item not found in cart"}, status=status.HTTP_404_NOT_FOUND
            )
        except Exception:
            return Response(
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            Order.objects.filter(models.Q(buyer=user) | models.Q(items__seller=user))
            .distinct()
            .prefetch_related("items", "shipping_info")
        )
    ...
    def update_status(self, request, pk=None):
        ...
        if not (
            request.user == order.buyer or order.items.filter(seller=request.user).exists() or request.user.is_staff
        ):
    ...
    def update_tracking(self, request, pk=None):
        ...
        if not (user_owns_items or is_staff):
            logger.warning(
                f"User {request.user.username} attempted to update tracking for order {order.id} but owns no items in this order"
            )
            raise PermissionDenied(
                "You don't have permission to update tracking for this order. You must be the seller of at least one item."
            )
    ...
    def seller_orders(self, request):
        ...
        seller_orders = (
            ...
            .prefetch_related("items", "shipping_info", "shipping_info__seller")
            .order_by("-created_at")
        )
        ...
        for order in seller_orders:
            ...
            logger.debug(
                f"Order {order.id}: Looking for seller {request.user.id}, found sellers: {all_sellers_in_order}"
            )

            seller_items = [item for item in order_data["items"] if item["seller"] == request.user.id]
            ...
            if not seller_items:
                logger.warning(
                    f"Order {order.id} has no items for seller {request.user.id}. Found sellers: {all_sellers_in_order}"
                )
                skipped_orders += 1
            ...
            seller_shipping = [
                shipping for shipping in order_data["shipping_info"] if shipping["seller"]["id"] == request.user.id
            ]
    ...
    def cancel_order(self, request, pk=None):
        ...
        return Response(
            {"success": True, "message": "Order cancelled successfully", "order": OrderSerializer(order).data}
        )
    ...
    def update_order_status(self, request, pk=None):
        ...
        if not (user_owns_items or is_buyer or is_staff):
            raise PermissionDenied(
                "You don't have permission to update this order. You must be the seller of at least one item."
            )
        ...
        logger.info(
            f"Order {order.id} status updated from {old_status} to {new_status} by user {request.user.username}"
        )
    ...
    def update_carrier_code(self, request, pk=None):
        ...
        if not carrier_code:
            return Response(
                {"detail": "Carrier code is required (e.g., CTT DY08912401385471)"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ProductMetricsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def dashboard_metrics(self, request):
        ...
        # Category count
        category_count = Category.objects.filter(products__seller=user, products__is_active=True).distinct().count()
            ...
            orders_data.append(
                {
                    ...
                    "customer": f"{order_item.order.buyer.first_name} {order_item.order.buyer.last_name}"
                    or order_item.order.buyer.username,
                    ...
                    "shipping_address": (
                        order_item.order.shipping_address.get("address", "N/A")
                        if order_item.order.shipping_address
                        else "N/A"
                    ),
                }
            )
    ...
    def product_metrics(self, request, product_slug=None):
        ...
        recent_orders = (
            OrderItem.objects.filter(product=product, seller=user)
            .select_related("order", "product")
            .order_by("-order__created_at")[:10]
        )
...
class UserProfileViewSet(viewsets.ViewSet):
    ...
    def profile_picture(self, request):
        ...
        if not getattr(settings, "USE_S3", False):
            return Response({"detail": "S3 storage is not enabled", "profile_picture": None})
        ...
        try:
            ...
            return Response(
                {"user_id": str(request.user.id), "username": request.user.username, "profile_picture": profile_pic}
            )

        except S3StorageError as e:
            logger.error(f"Failed to get profile picture for user {request.user.id}: {str(e)}")
            return Response(
                {"detail": f"Failed to retrieve profile picture: {str(e)}", "profile_picture": None},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def upload_profile_picture(self, request):
        ...
        try:
            ...
            result = s3_storage.upload_profile_picture(
                user_id=str(request.user.id), image_file=image_file, replace_existing=True
            )
            ...
            return Response(
                {"detail": f"Failed to upload profile picture: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def delete_profile_picture(self, request):
        ...
        try:
            ...
            if success:
                ...
                return Response({"detail": "No profile picture found to delete"}, status=status.HTTP_404_NOT_FOUND)

        except S3StorageError as e:
            logger.error(f"Failed to delete profile picture for user {request.user.id}: {str(e)}")
            return Response(
                {"detail": f"Failed to delete profile picture: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
def seller_profile(request, seller_id):
    ...
    try:
        ...
        if seller.role not in ["seller", "admin"]:
            return Response({"error": "User is not a seller"}, status=status.HTTP_404_NOT_FOUND)
        ...
        reviews = (
            ProductReview.objects.filter(product__seller=seller, is_active=True)
        ...
        total_sales = (
            OrderItem.objects.filter(product__seller=seller, order__status="delivered").aggregate(
                total=Sum("quantity")
            )["total"]
            or 0
        )
        ...
        return Response({"error": "Failed to fetch seller profile"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
