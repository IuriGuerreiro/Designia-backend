...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def dispatch(self, request, *args, **kwargs):
        warnings.warn(
            "ProductViewSet in views_legacy is deprecated. Use marketplace.views.ProductViewSet instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def _update_via_service(self, request, *args, **kwargs):
        ...
        if not result.ok:
            if result.error == ErrorCodes.NOT_PRODUCT_OWNER:
                return Response({"detail": result.error_detail}, status=status.HTTP_403_FORBIDDEN)
            elif result.error == ErrorCodes.PRODUCT_NOT_FOUND:
                return Response({"detail": result.error_detail}, status=status.HTTP_404_NOT_FOUND)
            return Response({"error": result.error, "detail": result.error_detail}, status=status.HTTP_400_BAD_REQUEST)
...
class ProductReviewViewSet(viewsets.ModelViewSet):
    ...
    def dispatch(self, request, *args, **kwargs):
        warnings.warn(
            "ProductReviewViewSet in views_legacy is deprecated. Use marketplace.views.ReviewViewSet instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        return super().dispatch(request, *args, **kwargs)
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def dispatch(self, request, *args, **kwargs):
        warnings.warn(
            "OrderViewSet in views_legacy is deprecated. Use marketplace.views.OrderViewSet instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        return super().dispatch(request, *args, **kwargs)
