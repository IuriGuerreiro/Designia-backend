...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('=== CREATING MISSING PRODUCT METRICS ===')
        )
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f"DRY RUN: Would create {missing_count} ProductMetrics records")
            )
        ...
        try:
            ...
            raise CommandError(f"Error creating ProductMetrics: {str(e)}")
        ...
        if products_still_missing == 0:
            self.stdout.write(
                self.style.SUCCESS("🎉 All products now have ProductMetrics!")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"⚠️  {products_still_missing} products still missing metrics")
            )
