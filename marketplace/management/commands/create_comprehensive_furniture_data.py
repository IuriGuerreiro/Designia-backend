...
class Command(BaseCommand):
    ...
    def _create_comprehensive_categories(self, category_filter: Optional[List[str]] = None) -> List[Category]:
        ...
        self.stdout.write("[CATEGORIES] Creating comprehensive category structure...")
        ...
        for _cat_key, cat_data in categories_structure.items():
            ...
            for subcat_name, subcat_desc in cat_data["subcategories"]:
                ...
                if sub_created:
                    self.stdout.write(f"    [CATEGORIES] Created subcategory: {subcategory.name}")

        self.stdout.write(f"[DONE] Created {len(created_categories)} categories")
        return created_categories

    def _create_diverse_users(self, count: int) -> List[User]:
        """Create diverse user profiles with different seller types."""
        self.stdout.write(f"[USERS] Creating {count} diverse users...")
    ...
    def _create_realistic_products(
        ...
        for i in range(count):
            if i % progress_interval == 0:
                ...
                self.stdout.write(f"  [STATS] Progress: {progress}% ({i}/{count} products)")
            ...
            try:
                ...
                if len(created_products) % batch_size == 0:
                    self.stdout.write(f"    [BATCH] Committed batch of {batch_size} products")

            except Exception as e:
                self.stdout.write(f"[WARNING] Failed to create product {name_variation}: {e}")

        self.stdout.write(f"[DONE] Created {len(created_products)} products")
    ...
    def _create_product_images(self, product: Product, image_urls: List[str]):
        ...
        try:
            ...
            self.stdout.write(f"[WARNING] Failed to create images for {product.name}: {e}")
    ...
    def _create_product_reviews(self, products: List[Product], users: List[User], count: int):
        ...
        self.stdout.write(f"[REVIEWS] Creating {count} product reviews...")
        ...
        self.stdout.write(f"[DONE] Created {len(created_reviews)} reviews")
    ...
    def _create_favorites(self, products: List[Product], users: List[User], count: int):
        ...
        self.stdout.write(f"[FAVORITES] Creating {count} favorite relationships...")
        ...
        self.stdout.write(f"[DONE] Created {len(created_favorites)} favorites")

    def _create_cart_items(self, products: List[Product], users: List[User], count: int):
        """Create realistic shopping cart items."""
        self.stdout.write(f"[CART] Creating {count} cart items...")
        ...
        self.stdout.write(f"[DONE] Created {len(created_cart_items)} cart items")
    ...
    def _update_product_metrics(self, products: List[Product]):
        ...
        self.stdout.write("[METRICS] Updating product metrics...")
...
│ CATEGORIES                              │
├─────────────────────────────────────────┤
│ Main Categories:        {len(main_categories):>3}            │
│ Subcategories:          {len(subcategories):>3}            │
│ Total Categories:       {len(categories):>3}            │
├─────────────────────────────────────────┤
│ USERS                                   │
├─────────────────────────────────────────┤
...
│ PRODUCTS                                │
├─────────────────────────────────────────┤
│ Total Products:         {len(products):>3}            │
│ With Images:            {ProductImage.objects.count():>3}            │
│ Featured Products:      {len([p for p in products if p.is_featured]):>3}            │
│ Price Range:       ${min_price} - ${max_price}     │
│ Average Price:          ${avg_price:.2f}         │
├─────────────────────────────────────────┤
│ ENGAGEMENT DATA                         │
├─────────────────────────────────────────┤
│ Product Reviews:        {total_reviews:>3}            │
│ Favorite Items:         {total_favorites:>3}            │
│ Shopping Carts:         {total_carts:>3}            │
│ Cart Items:             {total_cart_items:>3}            │
└─────────────────────────────────────────┘

READY FOR TESTING!
Your marketplace now has comprehensive, realistic furniture data ready for:
• Product browsing and search functionality
• User authentication and profile testing
...
• Test frontend marketplace functionality
• Verify search and filtering features
• Test user interactions and engagement
"""
