...
class UserClickAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Activity Information", {"fields": ("product", "action", "user", "session_key")}),
        ("Request Metadata", {"fields": ("ip_address", "user_agent", "referer"), "classes": ["collapse"]}),
...
class ActivitySummaryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Summary Information", {"fields": ("product", "period_type", "period_start", "period_end")}),
        (
            "Activity Metrics",
            {
                "fields": (
        ...
        ("Unique Metrics", {"fields": ("unique_users", "unique_sessions")}),
