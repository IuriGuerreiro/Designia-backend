...
def track_product_activity(request):
    ...
    try:
        ...
        activity = UserClick.track_activity(
            product=product, action=action, user=user, session_key=session_key, request=request
        )
        ...
        return Response({"error": f"Failed to track activity: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_product_activity_stats(request, product_id):
    ...
    try:
        ...
        for action, _ in UserClick.ACTION_CHOICES:
            activity_counts[action] = UserClick.objects.filter(product=product, action=action).count()
        ...
        return Response(
            {"error": f"Failed to get activity stats: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def get_user_activity_history(request):
    ...
    try:
        ...
        return Response({"activities": activity_data, "total_count": len(activity_data)}, status=status.HTTP_200_OK)

    except Exception as e:
        return Response(
            {"error": f"Failed to get activity history: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
