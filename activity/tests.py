...
from marketplace.models import Category, Product, ProductMetrics
...
class UserClickModelTest(TestCase):
    ...
    def test_track_activity_creates_click_record(self):
        ...
        activity = UserClick.track_activity(product=self.product, action="view", user=self.user)
    ...
    def test_track_activity_updates_product_metrics(self):
        ...
        # Track a view
        UserClick.track_activity(product=self.product, action="view", user=self.user)
        ...
        # Track a click
        UserClick.track_activity(product=self.product, action="click", user=self.user)
    ...
    def test_track_activity_for_anonymous_user(self):
        ...
        activity = UserClick.track_activity(product=self.product, action="view", session_key="anonymous_session_123")
    ...
    def test_favorite_unfavorite_updates_metrics_correctly(self):
        ...
        # Add favorite
        UserClick.track_activity(product=self.product, action="favorite", user=self.user)

        metrics.refresh_from_db()
        self.assertEqual(metrics.total_favorites, 1)

        # Remove favorite
        UserClick.track_activity(product=self.product, action="unfavorite", user=self.user)
...
class ActivitySummaryModelTest(TestCase):
    ...
    def test_generate_daily_summary(self):
        ...
        today = date.today()
        UserClick.objects.create(product=self.product, action="view", user=self.user)
        UserClick.objects.create(product=self.product, action="click", user=self.user)
        UserClick.objects.create(product=self.product, action="favorite", session_key="anonymous_123")
