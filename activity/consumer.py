...
class ActivityConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.user is None or isinstance(self.user, AnonymousUser):
            logger.warning("Unauthenticated WebSocket connection attempt to activity")
    ...
    async def handle_get_cart_count(self):
        ...
        cart_count = await self.get_user_cart_count()
        await self.send(text_data=json.dumps({"type": "cart_count_update", "cart_count": cart_count}))
    ...
    async def handle_get_unread_count(self):
        ...
        await self.send(
            text_data=json.dumps({"type": "unread_messages_count_update", "unread_messages_count": unread_count})
        )
    ...
    async def handle_track_activity(self, data):
        ...
        if not product_id or not action:
            await self.send(text_data=json.dumps({"type": "error", "message": "product_id and action are required"}))
            return
    ...
    async def unread_messages_count_update(self, event):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "unread_messages_count_update", "unread_messages_count": event["unread_count"]}
            )
        )
    ...
    def get_user_unread_messages(self):
        ...
        try:
            ...
            user_chats = Chat.objects.filter(models.Q(user1=self.user) | models.Q(user2=self.user)).values_list(
                "id", flat=True
            )
            ...
            unread_count = (
                ...
                .exclude(sender=self.user)  # Exclude own messages
                .count()
            )
    ...
    def track_user_activity(self, product_id, action):
        ...
        try:
            ...
            UserClick.track_activity(product=product, action=action, user=self.user)
            return True
        except Exception as e:
    ...
    async def notify_cart_update(
        user_id, action, product_id=None, cart_count=None, message=None, quantity_change=None
    ):
    ...
    async def notify_unread_count_update(user_id, unread_count):
        ...
        try:
            ...
            await channel_layer.group_send(
                user_group_name, {"type": "unread_messages_count_update", "unread_count": unread_count}
            )
